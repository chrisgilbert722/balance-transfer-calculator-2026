...
import { calculateBalanceTransfer } from './logic/balanceTransferCalculations';
import type { BalanceTransferInput } from './logic/balanceTransferCalculations';

function App() {
  const [values, setValues] = useState<BalanceTransferInput>({
    currentBalance: 5000,
    currentAPR: 24.99,
    newAPR: 0,
    monthlyPayment: 200,
  ...
  const handleChange = (field: keyof BalanceTransferInput, value: number | boolean) => {
    setValues(prev => ({ ...prev, [field]: value as number }));
  };

  const result = calculateBalanceTransfer(values);
